...
if($stmt = $mysqli->prepare("SELECT Titel, beschrijving FROM categorien WHERE id = 1")){
	$stmt->execute();
...
$query = "SELECT * FROM message WHERE categorie=1";
...
<div>
	...
	<?php if (login_check($mysqli)){
		$logged = true;?>
		<a href="?id=edit&msg_id=<?= $result['id']?>">Veranderen</a>
	<?php }?>
</div>
<?php }?>